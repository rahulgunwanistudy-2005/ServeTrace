	...
	<table class="w-full border-collapse text-left text-sm">
		...
		<thead class="border-b border-line bg-sunken">
			<tr class="text-xs tracking-wide text-muted uppercase">
				<th scope="col" class="px-4 py-3 font-semibold">{result.colClaim}</th>
				<th scope="col" class="px-4 py-3 font-semibold">{result.colWhen}</th>
				<th scope="col" class="px-4 py-3 font-semibold">{result.colDistance}</th>
				<th scope="col" class="px-4 py-3 font-semibold">{result.colVerdict}</th>
			</tr>
		</thead>
		<tbody>
			{#each verdicts as verdict (verdict.claim_ref)}
				<tr class="border-b border-line last:border-0">
					<th scope="row" class="px-4 py-3 font-medium">{label(verdict.claim_ref)}</th>
					<td class="px-4 py-3 text-muted">{formatDateTime(verdict.claimed_at)}</td>
					...
					<td class="px-4 py-3"><TierBadge tier={verdict.tier} size="sm" /></td>
				</tr>
			{/each}
		</tbody>
	</table>
